...
<style>
  ...
  /* Categoría padre */
  .cat-padre { font-weight: 800; color: #fff; font-size: 0.9rem; }
  .cat-desc { font-size: 0.75rem; color: rgba(255,255,255,0.3); margin-top: 0.1rem; }

  /* Subcategoría */
  .cat-sub-row { background: rgba(255,255,255,0.01); }
  .cat-sub-nombre {
    display: flex;
    align-items: center;
    gap: 0.5rem;
  ...
  .sub-count-badge { display: inline-flex; align-items: center; padding: 0.2rem 0.65rem; border-radius: 999px; font-size: 0.72rem; font-weight: 700; background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.3); border: 1px solid rgba(255,255,255,0.08); }
  ...
  /* Tip info */
  .tip-card { background: rgba(249,115,22,0.05); border: 1px solid rgba(249,115,22,0.15); border-radius: 12px; padding: 0.9rem 1.1rem; font-size: 0.78rem; color: rgba(255,255,255,0.4); display: flex; gap: 0.6rem; align-items: flex-start; }
  .tip-card strong { color: rgba(255,255,255,0.65); }
...
<div class="page-header">
  <div>
    ...
    <p class="page-count">{{ $categorias->count() }} categoría{{ $categorias->count() !== 1 ? 's' : '' }} principal{{ $categorias->count() !== 1 ? 'es' : '' }}</p>
  </div>
</div>
...
<div class="layout">
  ...
  <div class="table-card">
    <div class="card-section-title">Lista de categorías y subcategorías</div>
        ...
        <tbody>
          @forelse($categorias as $categoria)
            ...
            <tr>
              ...
              <td>
                <div class="cat-padre"> {{ $categoria->nombre }}</div>
                @if($categoria->descripcion)
                  <div class="cat-desc">{{ $categoria->descripcion }}</div>
                @endif
                @if($categoria->subcategorias->count() > 0)
                  <div class="cat-desc" style="margin-top:0.2rem; color:rgba(249,115,22,0.5);">
                    {{ $categoria->subcategorias->count() }} subcategoría{{ $categoria->subcategorias->count() !== 1 ? 's' : '' }}
                  </div>
                @endif
              </td>
              <td class="center">
            ...
            <tr class="cat-sub-row">
              <td><span class="id-chip" style="opacity:0.4;">{{ $sub->id }}</span></td>
              <td>
                <div class="cat-sub-nombre">
                  <div class="cat-sub-line"></div>
                   {{ $sub->nombre }}
                </div>
              </td>
              <td class="center">
                <span class="sub-count-badge">{{ $sub->productos()->count() }} productos</span>
              </td>
              <td class="center">
                <div class="actions-wrap">
                  <form action="{{ route('categorias.destroy', $sub) }}" method="POST" class="form-delete">
                    @csrf @method('DELETE')
  ...
  <div class="side-panel">
    ...
    {{-- Tip --}}
    <div class="tip-card">
      💡 <span>Para crear una <strong>subcategoría</strong>, selecciona una categoría padre en el campo de abajo.</span>
    </div>
    ...
    <div class="form-card">
      ...
      <div style="padding:1.25rem 1.4rem;">
        <form method="POST" action="{{ route('categorias.store') }}">
          ...
          {{-- Categoría padre --}}
          <div class="field-group">
            <label class="field-label" for="parent_id">
              Es subcategoría de
              <span style="color:rgba(255,255,255,0.2); font-weight:400;">(opcional)</span>
            </label>
            <div class="field-wrap">
